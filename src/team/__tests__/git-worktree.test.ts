...
describe('git-worktree', () => {
  ...
  beforeEach(() => {
    ...
    execFileSync('git', ['config', 'user.email', 'hi@coli.dev'], { cwd: repoDir, stdio: 'pipe' });
  ...
  describe('createWorkerWorktree', () => {
    it('creates worktree at correct path', () => {
      ...
      expect(info.path).toContain('.omd/worktrees');
      expect(info.branch).toBe(`omd-team/${teamName}/worker1`);
    ...
    it('branch name is properly sanitized', () => {
      ...
      expect(info.branch).toContain('omd-team/');
      expect(existsSync(info.path)).toBe(true);
    });
  ...
  describe('removeWorkerWorktree', () => {
    it('removes worktree and branch', () => {
      ...
      expect(branches).not.toContain('omd-team/');
    });
