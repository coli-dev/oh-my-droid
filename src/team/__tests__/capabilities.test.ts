...
function makeMember(
  name: string,
  backend: 'droid-native' | 'mcp-codex' | 'mcp-gemini',
...
describe('capabilities', () => {
  describe('getDefaultCapabilities', () => {
    it('returns capabilities for droid-native', () => {
      const caps = getDefaultCapabilities('droid-native');
    ...
    it('returns a copy, not a reference', () => {
      const caps1 = getDefaultCapabilities('droid-native');
      const caps2 = getDefaultCapabilities('droid-native');
      caps1.push('research');
  ...
  describe('scoreWorkerFitness', () => {
    ...
    it('gives partial credit for general capability', () => {
      const worker = makeMember('w1', 'droid-native', ['general']);
    ...
    it('returns 1.0 when no capabilities required', () => {
      const worker = makeMember('w1', 'droid-native', ['code-edit']);
  ...
  describe('rankWorkersForTask', () => {
    it('ranks workers by fitness score descending', () => {
      ...
      const w3 = makeMember('droid', 'droid-native', ['code-edit', 'testing', 'general']);
    ...
    it('respects custom capabilities over defaults', () => {
      const w1 = makeMember('custom', 'droid-native', ['security-review', 'architecture']);
