...
Handles OMD initialization and maintenance tasks.

## Triggers

### `init`
Initializes OMD directory structure and environment on first run or explicit setup.
...
- Sets environment variables (`OMD_INITIALIZED=true`) if `DROID_ENV_FILE` is available
...
```json
{
  ...
  "hookSpecificOutput": {
    "hookEventName": "Setup",
    "additionalContext": "OMD initialized:\n- 5 directories created\n- 1 configs validated\n- Environment variables set: OMD_INITIALIZED"
  }
}
```

### `maintenance`
Performs periodic maintenance tasks to keep OMD state clean.
...
Creates all required OMD directories.
...
Sets environment variables for OMD initialization.
...
- Environment variable setting requires `DROID_ENV_FILE` to be set
