...
function createTempDir(): string {
  ...
  mkdirSync(join(dir, '.omd', 'state'), { recursive: true });
  return dir;
}
...
describe('processPreCompact - Compaction Mutex (issue #453)', () => {
  ...
  it('should only create one checkpoint file per coalesced batch', async () => {
    ...
    const checkpointDir = join(tempDir, '.omd', 'state', 'checkpoints');
  ...
  it('should handle concurrent calls for different directories independently', async () => {
    ...
    try {
      ...
      const checkpointDir1 = join(tempDir, '.omd', 'state', 'checkpoints');
      const checkpointDir2 = join(tempDir2, '.omd', 'state', 'checkpoints');
