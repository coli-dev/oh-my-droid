 ...
 * Supports Droid, GPT, and Gemini model families.
 *
...
const THINKING_CAPABLE_MODELS: Record<string, readonly string[]> = {
  ...
  'amazon-bedrock': ['droid', 'anthropic'],
...
function resolveProvider(providerId: string, modelId: string): string {
  ...
  if (providerId === 'github-copilot') {
    ...
    if (modelLower.includes('droid') || modelLower.includes('claude')) return 'anthropic';
...
/**
 * Get Droid-specific thinking configuration.
 * This is used by Droid for extended thinking.
 */
export function getDroidThinkingConfig(budgetTokens: number = 64000) {
  return {
    thinking: {
