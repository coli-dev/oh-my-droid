 ...
 *
 * @see https://docs.anthropic.com/en/docs/droid-code (check for updates)
 */
 ...
 * for compatibility with different Droid versions.
 *
 * IMPORTANT: The abort detection patterns below are assumed. Verify
 * actual stop_reason values from Droid before finalizing.
 */
export interface StopContext {
  /** Reason for stop (from Droid) - snake_case variant */
  stop_reason?: string;
  /** Reason for stop (from Droid) - camelCase variant */
  stopReason?: string;
 ...
 * should be updated based on observed Droid behavior.
 */
...
function getTodoFilePaths(sessionId?: string, directory?: string): string[] {
  ...
  if (sessionId) {
    paths.push(join(droidDir, 'sessions', sessionId, 'todos.json'));
    paths.push(join(droidDir, 'todos', `${sessionId}.json`));
  }
...
export function readTaskFiles(sessionId: string): Task[] {
  ...
  try {
    for (const file of readdirSync(taskDir)) {
      // Skip non-JSON files and .lock file (used by Droid for atomic writes)
