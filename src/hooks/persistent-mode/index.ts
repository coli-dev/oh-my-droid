 ...
 * 2. Active ralph loop (until cancelled via /oh-my-droid:cancel)
...
function checkArchitectApprovalInTranscript(sessionId: string): boolean {
  ...
  const possiblePaths = [
    ...
    join(droidDir, 'transcripts', `${sessionId}.md`)
  ];
...
4. When FULLY complete (after Architect verification), run \`/oh-my-droid:cancel\` to cleanly exit and clean up state files. If cancel fails, retry with \`/oh-my-droid:cancel --force\`.
