 ...
 * MCP Server Bridge
 *
 * Connects to MCP servers and exposes their tools to OMD.
 * Handles:
...
export class McpBridge extends EventEmitter {
  ...
  async connect(serverName: string): Promise<ExternalTool[]> {
    ...
    const safeEnv: Record<string, string> = { ...process.env } as Record<
      string,
      string
    >;
    if (config.env) {
  ...
  private sendRequest(
    ...
    if (!connection) {
      return Promise.reject(
        new Error(`Not connected to server: ${serverName}`),
      );
    }
