...
describe("MCP Server Workflows", () => {
      ...
      vi.mocked(spawn).mockImplementation(
        (cmd: string, args: readonly string[]) => {
          ...
          return {
            stdout: {
              on: vi.fn((e: string, cb: (...args: unknown[]) => void) => {
                if (e === "data") cb("Large context analysis");
              }),
            },
            stderr: { on: vi.fn() },
            on: vi.fn((e: string, cb: (...args: unknown[]) => void) => {
      ...
      vi.mocked(spawn).mockImplementation(
        (cmd: string, args: readonly string[]) => {
          ...
          return {
            stdout: {
              on: vi.fn((e: string, cb: (...args: unknown[]) => void) => {
                if (e === "data") cb("Pro model response");
              }),
            },
            stderr: { on: vi.fn() },
            on: vi.fn((e: string, cb: (...args: unknown[]) => void) => {
  ...
  describe("Timeout and Environment Configuration", () => {
    it("should respect OMD_CODEX_TIMEOUT environment variable", async () => {
      const originalTimeout = process.env.OMD_CODEX_TIMEOUT;
      process.env.OMD_CODEX_TIMEOUT = "120000";
      ...
      const { codexMcpServer: freshServer } =
        await import("../mcp/codex-server.js");
      expect(freshServer).toBeDefined();

      process.env.OMD_CODEX_TIMEOUT = originalTimeout;
    });

    it("should respect OMD_GEMINI_TIMEOUT environment variable", async () => {
      const originalTimeout = process.env.OMD_GEMINI_TIMEOUT;
      process.env.OMD_GEMINI_TIMEOUT = "180000";
      ...
      const { geminiMcpServer: freshServer } =
        await import("../mcp/gemini-server.js");
      expect(freshServer).toBeDefined();

      process.env.OMD_GEMINI_TIMEOUT = originalTimeout;
    });
  });
});
