...
describe("Discovery System", () => {
  ...
  describe("discoverMcpServers", () => {
    it("should discover MCP servers from droid_desktop_config.json", () => {
      createTestMcpConfig({
        "test-server": {
          command: "npx",
          args: ["-y", "test-server"],
      ...
      expect(servers[0].source).toBe("droid_desktop_config");
    });
    ...
    it("should prioritize settings.json over droid_desktop_config.json", () => {
      createTestSettings({
        "shared-server": {
...
describe("Permission Adapter", () => {
  ...
  describe("checkPermission", () => {
    ...
    it("should require permission for write operations", () => {
      const result = checkPermission("mcp__filesystem__write_file", {
        path: "/test",
      });
