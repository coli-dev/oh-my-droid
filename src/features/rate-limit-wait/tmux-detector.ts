 ...
 * tmux Detector
 *
 * Detects Droid sessions running in tmux panes and identifies
...
/** Patterns that indicate Droid is running */
const DROID_CODE_PATTERNS = [
  /droid/i,
  /\$ droid/,
  /droid/i,
  /anthropic/i,
...
/**
 * Analyze pane content to determine if it shows a rate-limited Droid session
 */
export function analyzePaneContent(content: string): PaneAnalysisResult {
  ...
  // Check for Droid indicators
  const hasDroidCode = DROID_CODE_PATTERNS.some((pattern) =>
  ...
  let confidence = 0;
  if (hasDroidCode) confidence += 0.4;
  ...
  const isBlocked = hasDroidCode && hasRateLimitMessage && confidence >= 0.6;
...
/**
 * Scan all tmux panes for blocked Droid sessions
 *
...
export function formatBlockedPanesSummary(blockedPanes: BlockedPane[]): string {
  if (blockedPanes.length === 0) {
    return 'No blocked Droid sessions detected.';
  }

  const lines: string[] = [
    `Found ${blockedPanes.length} blocked Droid session(s):`,
    '',
  ];
