 ...
 * Droid sessions when rate limits reset.
 *
...
const DEFAULT_CONFIG: Required<DaemonConfig> = {
  ...
  verbose: false,
  stateFilePath: join(homedir(), '.omd', 'state', 'rate-limit-daemon.json'),
  pidFilePath: join(homedir(), '.omd', 'state', 'rate-limit-daemon.pid'),
  logFilePath: join(homedir(), '.omd', 'state', 'rate-limit-daemon.log'),
};
...
export async function runDaemonForeground(config?: DaemonConfig): Promise<void> {
  ...
  // Check if already running
  if (isDaemonRunning(cfg)) {
    console.error('Daemon is already running. Use "omd wait daemon stop" first.');
    process.exit(1);
  }
