...
async function runQuickBackfill(silent: boolean = false): Promise<void> {
  ...
  if (result.entriesAdded > 0 && !silent) {
    console.log(
      chalk.green(
        `Backfilled ${result.entriesAdded} entries in ${result.timeElapsed}ms`,
      ),
    );
  }
}
...
async function displayAnalyticsBanner() {
  try {
    ...
    const banner = gradient.default.pastel.multiline(
      [
        "╔═══════════════════════════════════════╗",
        "║   Oh-My-DroidCode - Analytics Dashboard   ║",
        "╚═══════════════════════════════════════╝",
      ].join("\n"),
    );
    console.log(banner);
    ...
    console.log("╔═══════════════════════════════════════╗");
    console.log("║   Oh-My-DroidCode - Analytics Dashboard   ║");
    console.log("╚═══════════════════════════════════════╝");
    console.log("");
  }
}
...
async function defaultAction() {
  ...
  if (tuiAvailable) {
    ...
    console.log(
      chalk.dim(
        "Tip: Run `omd tui` for an interactive token visualization dashboard",
      ),
    );
  }
}
...
program
  .name("omd")
  .description(
    "Multi-agent orchestration system for Droid Agent SDK with analytics",
  )
  .version(version)
...
program
  ...
  .option(
    "--session <id>",
    "Show stats for specific session (defaults to aggregate)",
  )
  .addHelpText(
    "after",
    `
Examples:
  $ omd stats                    Show aggregate statistics
  $ omd stats --session abc123   Show stats for a specific session
  $ omd stats --json             Output as JSON for scripting`,
  )
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd cost                     Show monthly cost report
  $ omd cost daily               Show daily cost breakdown
  $ omd cost weekly --json       Export weekly costs as JSON`,
  )
  .action(async (period = "monthly", options) => {
    if (!["daily", "weekly", "monthly"].includes(period)) {
      console.error(
        chalk.red(
          `Invalid period "${period}". Valid options: daily, weekly, monthly`,
        ),
      );
      console.error(chalk.gray("Example: omd cost weekly"));
      process.exit(1);
    }
    await ensureBackfillDone();
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd sessions                 Show last 10 sessions
  $ omd sessions --limit 50      Show last 50 sessions
  $ omd sessions --json          Export session history as JSON`,
  )
  .action(async (options) => {
    await ensureBackfillDone();
...
program
  ...
  .description(
    "Export data (type: cost, sessions, patterns; format: json, csv)",
  )
  .option(
    "--period <period>",
    "Period for cost report (daily, weekly, monthly)",
    "monthly",
  )
  .addHelpText(
    "after",
    `
Examples:
  $ omd export cost json costs.json           Export monthly costs to JSON
  $ omd export sessions csv sessions.csv      Export sessions to CSV
  $ omd export cost csv data.csv --period weekly   Export weekly costs`,
  )
  .action((type, format, output, options) => {
    if (!["cost", "sessions", "patterns"].includes(type)) {
      console.error(
        chalk.red(
          `Invalid type "${type}". Valid options: cost, sessions, patterns`,
        ),
      );
      console.error(chalk.gray("Example: omd export cost json output.json"));
      process.exit(1);
    }
    if (!["json", "csv"].includes(format)) {
      console.error(
        chalk.red(`Invalid format "${format}". Valid options: json, csv`),
      );
      console.error(
        chalk.gray("Example: omd export sessions csv sessions.csv"),
      );
      process.exit(1);
    }
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd cleanup                  Clean up logs older than 30 days
  $ omd cleanup --retention 7    Clean up logs older than 7 days`,
  )
  .action((options) => {
...
program
  .command("backfill")
  .description(
    "[DEPRECATED] Backfill now runs automatically. Use for manual re-sync only.",
  )
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd backfill --reset                       Force full re-sync
  $ omd backfill --project ~/myproject         Backfill specific project
  $ omd backfill --from 2024-01-01 --verbose   Backfill from date with progress`,
  )
  .action(async (options) => {
    if (!options.reset && !options.project && !options.from && !options.to) {
      console.log(
        chalk.yellow(
          "Note: Backfill now runs automatically with every omd command.",
        ),
      );
      console.log(
        chalk.gray(
          "Use --reset to force full re-sync, or --project/--from/--to for filtered backfill.\n",
        ),
      );
    }
...
program
  ...
  .option("--no-droid", "Show all providers (not just Droid)")
  .addHelpText(
    "after",
    `
Examples:
  $ omd tui                      Launch interactive dashboard
  $ omd tui --light              Use light theme
  $ omd tui --daily              Start with daily view`,
  )
  .action(async (options) => {
    ...
    try {
      ...
      console.error(
        chalk.gray(
          'Try running "omd tui" again, or check if tokscale is properly installed.',
        ),
      );
      process.exit(1);
    }
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd init                     Initialize in current directory
  $ omd init --global            Initialize global configuration
  $ omd init --force             Overwrite existing config`,
  )
  .action(async (options) => {
    console.log(
      chalk.yellow("⚠️  DEPRECATED: The init command is deprecated."),
    );
    console.log(
      chalk.gray(
        "Configuration is now managed automatically. Use /oh-my-droid:omd-setup instead.\n",
      ),
    );
    ...
    console.log(chalk.blue("Oh-My-DroidCode Configuration Setup\n"));

    // Check if config already exists
    if (existsSync(targetPath) && !options.force) {
      console.log(
        chalk.yellow(`Configuration already exists at ${targetPath}`),
      );
    ...
    const configContent = `// Oh-My-DroidCode Configuration
...
`;
    ...
    console.log(
      chalk.gray("Edit the configuration file to customize your setup."),
    );
...
Examples:
  ...
  $ omd config --validate        Validate configuration files
  $ omd config --paths           Show config file locations`,
  )
  .action(async (options) => {
    if (options.paths) {
      const paths = getConfigPaths();
...
Examples:
  ...
  $ omd config-stop-callback file --show`,
  )
      ...
      case "file": {
        ...
        config.stopHookCallbacks.file = {
          ...
          path:
            options.path ??
            current?.path ??
            "~/.factory/session-logs/{session_id}.md",
          format:
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd test-prompt "ultrawork fix bugs"    See how magic keywords are detected
  $ omd test-prompt "analyze this code"     Test prompt enhancement`,
  )
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd update                   Check and install updates
  $ omd update --check           Only check, don't install
  $ omd update --force           Force reinstall`,
  )
  .action(async (options) => {
    if (!options.quiet) {
      console.log(chalk.blue("Oh-My-DroidCode Update\n"));
    }
    ...
    try {
      ...
      console.error(
        chalk.gray(
          'Try again with "omd update --force", or reinstall with "omd install --force".',
        ),
      );
      process.exit(1);
    }
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd version                  Show version, install method, and commit hash`,
  )
  .action(async () => {
    const installed = getInstalledVersion();

    console.log(chalk.blue.bold("\nOh-My-DroidCode Version Information\n"));
    console.log(chalk.gray("━".repeat(50)));
...
Examples:
  ...
  .action(async (options) => {
    if (!options.quiet) {
      ...
      console.log(
        chalk.blue(
          "║         Oh-My-DroidCode Installer                        ║",
        ),
      );
      console.log(
        chalk.blue("║   Multi-Agent Orchestration for Droid               ║"),
      );
      console.log(
        chalk.blue(
          "╚═══════════════════════════════════════════════════════════╝",
    ...
    if (result.success) {
      if (!options.quiet) {
        ...
        console.log(
          "  /sisyphus-default             # Configure for current project",
        );
        ...
        console.log(
          "  Run '/sisyphus-default' (project) or '/sisyphus-default-global' (global)",
        );
        ...
        console.log("  1. Run 'droid' to start Droid");
        console.log(
          "  2. Type '/sisyphus-default' for project or '/sisyphus-default-global' for global",
        );
      ...
      console.error(
        chalk.gray('\nTry "omd install --force" to overwrite existing files.'),
      );
      console.error(
        chalk.gray('For more diagnostics, run "omd doctor conflicts".'),
      );
      process.exit(1);
    }
  });
...
const waitCmd = program
  .command("wait")
  .description(
    'Rate limit wait and auto-resume (just run "omd wait" to get started)',
  )
  .option("--json", "Output as JSON")
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd wait                     Show status and suggestions
  $ omd wait --start             Start auto-resume daemon
  $ omd wait --stop              Stop auto-resume daemon
  $ omd wait status              Show detailed rate limit status
  $ omd wait detect              Scan for blocked tmux sessions`,
  )
  .action(async (options) => {
...
waitCmd
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd wait daemon start            Start background daemon
  $ omd wait daemon stop             Stop the daemon
  $ omd wait daemon start -f         Run in foreground`,
  )
  .action(async (action: string, options) => {
    if (action !== "start" && action !== "stop") {
      ...
      console.error(chalk.gray("Example: omd wait daemon start"));
      process.exit(1);
    }
...
waitCmd
  .command("detect")
  .description("Scan for blocked Droid sessions in tmux")
  .option("--json", "Output as JSON")
 ...
 *
 * Usage:
 * - `omd teleport #123` - Create worktree for issue/PR #123
 * - `omd teleport my-feature` - Create worktree for feature branch
 * - `omd teleport list` - List existing worktrees
 * - `omd teleport remove <path>` - Remove a worktree
 */
const teleportCmd = program
  .command("teleport [ref]")
  .description(
    "Create git worktree for isolated development (e.g., omd teleport #123)",
  )
  .option(
    "--worktree",
  ...
  .option(
    "-p, --path <path>",
    "Custom worktree path (default: ~/Workspace/omd-worktrees/)",
  )
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd teleport #42             Create worktree for issue/PR #42
  $ omd teleport add-auth        Create worktree for a feature branch
  $ omd teleport list            List existing worktrees
  $ omd teleport remove ./path   Remove a worktree`,
  )
  .action(async (ref: string | undefined, options) => {
    if (!ref) {
      ...
      console.log("Usage:");
      console.log(
        "  omd teleport <ref>           Create worktree for issue/PR/feature",
      );
      console.log("  omd teleport list            List existing worktrees");
      console.log("  omd teleport remove <path>   Remove a worktree");
      console.log("");
      ...
      console.log(
        "  omd teleport #42             Create worktree for issue #42",
      );
      console.log(
        '  omd teleport add-auth        Create worktree for feature "add-auth"',
      );
      console.log("");
      return;
    }
...
teleportCmd
  .command("list")
  .description("List existing worktrees in ~/Workspace/omd-worktrees/")
  .option("--json", "Output as JSON")
  .action(async (options) => {
...
const doctorCmd = program
  .command("doctor")
  .description("Diagnostic tools for troubleshooting OMD installation")
  .addHelpText(
    "after",
    `
Examples:
  $ omd doctor conflicts         Check for plugin conflicts`,
  );
...
doctorCmd
  ...
  .addHelpText(
    ...
    `
Examples:
  $ omd doctor conflicts         Check for configuration issues
  $ omd doctor conflicts --json  Output results as JSON`,
  )
  .action(async (options) => {
...
program
  ...
  .action(async () => {
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray('  Run "oh-my-droid-sisyphus info" to see available droids.'),
      );
      console.log(
        chalk.yellow(
          '  Run "/sisyphus-default" (project) or "/sisyphus-default-global" (global) in Droid.',
        ),
      );
    } else {
      // Don't fail the npm install, just warn
      console.warn(
        chalk.yellow("⚠ Could not complete Sisyphus setup:"),
        result.message,
      );
      console.warn(
        chalk.gray('  Run "oh-my-droid-sisyphus install" manually to complete setup.'),
      );
    }
  });
