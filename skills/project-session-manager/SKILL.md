...
> **Quick Start:** For simple worktree creation without tmux sessions, use `omd teleport`:
> ```bash
> omd teleport #123          # Create worktree for issue/PR
> omd teleport my-feature    # Create worktree for feature
> omd teleport list          # List worktrees
> ```
> See [Teleport Command](#teleport-command) below for details.

Automate isolated development environments using git worktrees and tmux sessions with Droid. Enables parallel work across multiple tasks, projects, and repositories.
...
8. **Launch Droid** (unless --no-droid):
   ```bash
...
The `omd teleport` command provides a lightweight alternative to full PSM sessions. It creates git worktrees without tmux session management — ideal for quick, isolated development.
...
omd teleport #123
omd teleport owner/repo#123
omd teleport https://github.com/owner/repo/issues/42

# Create worktree for a feature
omd teleport my-feature

# List existing worktrees
omd teleport list

# Remove a worktree
omd teleport remove issue/my-repo-123
omd teleport remove --force feat/my-repo-my-feature
```
...
| `--path <path>` | Custom worktree root directory | `~/Workspace/omd-worktrees/` |
