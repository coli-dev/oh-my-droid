...
Examples:
  psm review omd#123
  psm fix coli-dev/oh-my-droid#42
  psm feature omd add-webhooks
  psm list
...
cmd_review() {
    ...
    if [[ "$no_tmux" != "true" ]]; then
        ...
        if [[ "$tmux_status" == "error" ]]; then
            ...
            # Launch Droid
            if [[ "$no_droid" != "true" ]]; then
                log_info "Launching Droid..."
                psm_launch_droid "$session_name"
            fi
        fi
    fi
