...
- Cancel with `/oh-my-droid:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   - Remove `.omd/state/autopilot-state.json`, `ralph-state.json`, `ultrawork-state.json`, `ultraqa-state.json`
   - Run `/oh-my-droid:cancel` for clean exit
</Steps>
...
If autopilot was cancelled or failed, run `/oh-my-droid:autopilot` again to resume from where it stopped.
...
**Stuck in a phase?** Check TODO list for blocked tasks, review `.omd/autopilot-state.json`, or cancel and resume.
