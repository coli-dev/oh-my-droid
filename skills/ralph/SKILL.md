...
7. **On approval**: Run `/oh-my-droid:cancel` to cleanly exit and clean up all state files
...
```
Task(subagent_type="oh-my-droid:executor-low", model="haiku", prompt="Add type export for UserConfig")
Task(subagent_type="oh-my-droid:executor", model="sonnet", prompt="Implement the caching layer for API responses")
Task(subagent_type="oh-my-droid:executor-high", model="opus", prompt="Refactor auth module to support OAuth2 flow")
```
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/oh-my-droid:cancel`
...
- [ ] `/oh-my-droid:cancel` run for clean state cleanup
</Final_Checklist>
...
### PRD Workflow
1. Create `.omd/prd.json` and `.omd/progress.txt`
...
Workflow: Detect flag, extract task, create `.omd/prd.json`, create `.omd/progress.txt`, begin ralph loop.
