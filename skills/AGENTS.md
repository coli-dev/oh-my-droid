...
Skills are reusable workflow templates that can be invoked via `/oh-my-droid:skill-name`. Each skill provides:
...
| `omd-setup/SKILL.md` | omd-setup | One-time setup wizard |
...
```bash
# Manual invocation
/oh-my-droid:skill-name

# With arguments
/oh-my-droid:skill-name arg1 arg2
...
- Test skill invocation with `/oh-my-droid:skill-name`
...
| Utility | learner, note, cancel, hud, doctor, omd-setup, mcp-setup, help | "stop", "cancel" |
