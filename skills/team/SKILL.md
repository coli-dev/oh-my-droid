...
Write OMD state for cancellation support:

```json
// .omd/state/team-state.json
{
  "active": true,
  "team_name": "fix-ts-errors",
  "agent_count": 3,
...
**Key difference from Droid teammates:**
- MCP workers operate via CLI, not Droid's tool system
...
|-----------|-----------|-----|
| Iterative multi-step work | Droid teammate | Needs tool-mediated iteration + team communication |
| Code review / security audit | Codex MCP | Specialized, cheaper than Droid opus |
...
| Build/test iteration loops | Droid teammate | Needs Bash tool + iterative fix cycles |
| Tasks needing team coordination | Droid teammate | Needs SendMessage for status updates |
...
Task decomposition:
#1 [mcp_codex] Security review of current auth code -> output to .omd/research/auth-security.md
...
#4 [droid_worker] Update auth tests + fix integration issues
#5 [mcp_codex] Final code review of all changes
```

The lead runs #1 (Codex analysis), then #2 and #3 in parallel (Codex refactors backend, Gemini redesigns frontend), then #4 (Droid teammate handles test iteration), then #5 (final Codex review).
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for MCP workers that cannot use Droid's team messaging tools.
...
The `/oh-my-droid:cancel` skill handles team cleanup:

1. Read `.omd/state/team-state.json` to get the `team_name`
...
Optional settings via `.omd-config.json`:
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omd/worktrees/{team}/{worker}` with branch `omd-team/{teamName}/{workerName}`.
