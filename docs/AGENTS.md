...
Examples:
- `Task(subagent_type="oh-my-droid:architect", model="haiku", prompt="Summarize this module boundary.")`
- `Task(subagent_type="oh-my-droid:executor", model="sonnet", prompt="Add input validation to the login flow.")`
- `Task(subagent_type="oh-my-droid:executor", model="opus", prompt="Refactor auth/session handling across the API layer.")`
</model_routing>
...
Notepad (session memory at `{worktree}/.omd/notepad.md`):
...
Project Memory (persistent at `{worktree}/.omd/project-memory.json`):
...
<skills>
Skills are user-invocable commands (`/oh-my-droid:<name>`). When you detect trigger patterns, invoke the corresponding skill.
...
<cancellation>
Hooks cannot read your responses -- they only check state files. You need to invoke `/oh-my-droid:cancel` to end execution modes. Use `--force` to clear all state files.
...
- `{worktree}/.omd/project-memory.json` -- project memory
- `{worktree}/.omd/plans/` -- planning documents
- `{worktree}/.omd/research/` -- research outputs
- `{worktree}/.omd/logs/` -- audit logs
</worktree_paths>
...
Say "setup omd" or run `/oh-my-droid:omd-setup`. Everything is automatic after that.
