...
Compaction-resilient memory system at `.omd/notepad.md` with three tiers:
...
**User skill:** `/oh-my-droid:note`
- `/oh-my-droid:note <content>` - Add to Working Memory
- `/oh-my-droid:note --priority <content>` - Add to Priority Context
...
**State files:**
- `.omd/state/ultrapilot-state.json` - Session state
...
**Usage:** `/cancel` or just say "cancelomd", "stopomd"
...
- Primary: `.omd/state/{name}.json` (local, per-project)
- Global backup: `~/.omd/state/{name}.json` (global, session continuity)
