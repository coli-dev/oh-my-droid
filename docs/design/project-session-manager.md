...
Project Session Manager (PSM) automates the creation and management of isolated development environments using git worktrees and tmux sessions with Droid. It enables parallel work across multiple tasks, projects, and repositories while maintaining clean separation and easy context switching.
...
1. **Context Switching Overhead**: Switching between tasks requires stashing changes, switching branches, and losing Droid context
...
- Spawn pre-configured tmux sessions with Droid
...
```bash
# Review PR #123 from oh-my-droid repo
/psm review omd#123

# Review PR from any GitHub URL
/psm review https://github.com/anthropics/droid-code/pull/456

# Review with specific focus
/psm review omd#123 --focus "security implications"
```

**What happens:**
1. Fetches PR branch
2. Creates worktree at `~/.psm/worktrees/omd/pr-123`
3. Spawns tmux session `psm:omd:pr-123`
4. Launches Droid with PR context pre-loaded
...
/psm fix omd#42 --branch fix/auth-timeout

# Fix from issue URL
/psm fix https://github.com/anthropics/droid-code/issues/789
```

**What happens:**
1. Fetches issue details via `gh`
2. Creates feature branch from main
3. Creates worktree at `~/.psm/worktrees/omd/issue-42`
4. Spawns tmux session with issue context
5. Pre-populates Droid with issue description

### 2.3 Feature Development

```bash
# Start new feature
/psm feature omd "add-webhook-support"

# Feature from existing branch
/psm feature omd --branch feature/webhooks

# Feature with specific base
/psm feature omd "dark-mode" --base develop
```
...
```bash
# Prepare release
/psm release omd v3.5.0

# Release candidate
/psm release omd v3.5.0-rc1 --draft

# Hotfix release
/psm release omd v3.4.1 --hotfix --base v3.4.0
```
...
| `--no-droid` | Skip Droid launch | false |
...
anthropics/droid-code#123

# GitHub URL
https://github.com/anthropics/droid-code/pull/123
...
// ~/.psm/projects.json
{
  "aliases": {
    "omd": {
      "repo": "anthropics/oh-my-droid",
      "local": "~/Workspace/oh-my-droid",
      "default_base": "main"
    },
    "cc": {
      "repo": "anthropics/droid-code",
      "local": "~/Workspace/droid-code",
      "default_base": "main"
    },
    "myapp": {
      "repo": "myorg/myapp",
...
└─────────────────────────────────────────────────────────────┘
    ...
    └─────────────────────────────────────────────────────────┘
              ...
              ▼               ▼               ▼
    ┌─────────────────┐ ┌─────────────┐ ┌─────────────────┐
    │ Worktree Manager│ │Tmux Manager │ │ Droid Launcher │
    │   (git cmd)     │ │ (tmux cmd)  │ │  (droid cmd)   │
    └─────────────────┘ └─────────────┘ └─────────────────┘
              │               │               │
              └───────────────┼───────────────┘
                              ▼
    ┌─────────────────────────────────────────────────────────┐
    │                    Integration Layer                     │
    │  (gh CLI, git, tmux, droid, omd skills, Clawdbot)       │
    └─────────────────────────────────────────────────────────┘
```
...
└────────────┘     └────────────┘     └────────────┘     └────────────┘
      ...
      │                  │                  │                  │
      ▼                  ▼                  ▼                  ▼
  - Fetch refs      - Droid active    - Session saved    - Worktree kept
...
├── .psm-context.md          # Pre-loaded Droid context
├── <project files>          # Actual code
└── .omd/                    # OMD state (if applicable)
```
...
// .psm-session.json
{
  ...
  "tmux_session": "psm:omd:pr-123",
  "worktree_path": "~/.psm/worktrees/omd/pr-123",
  "source_repo": "~/Workspace/oh-my-droid",
  "github": {
    "pr_number": 123,
    "pr_title": "Add webhook support",
    "pr_author": "contributor",
    "pr_url": "https://github.com/anthropics/oh-my-droid/pull/123"
  },
  "state": "active",
  "notes": []
}
...
|------|---------|---------|
| PR Review | `psm:<proj>:pr-<num>` | `psm:omd:pr-123` |
| Issue Fix | `psm:<proj>:issue-<num>` | `psm:omd:issue-42` |
| Feature | `psm:<proj>:feat-<name>` | `psm:omd:feat-auth` |
| Release | `psm:<proj>:rel-<ver>` | `psm:omd:rel-v3.5.0` |
| Generic | `psm:<proj>:<name>` | `psm:omd:experiment` |
...
// ~/.psm/sessions.json
{
  ...
  "sessions": {
    ...
    "omd:issue-42": {
      ...
      "worktree": "~/.psm/worktrees/omd/issue-42",
      "tmux": "psm:omd:issue-42",
      "type": "fix",
      "metadata": {
        "issue_number": 42,
        "issue_closed": false
...
```bash
# VSCode
/psm review omd#123 --editor vscode

# Cursor
/psm review omd#123 --editor cursor

# Neovim
/psm review omd#123 --editor nvim
```
...
PSM status in OMD HUD statusline:

```
[psm:omd:pr-123] 📋 Review | 🕐 2h active | 📁 ~/.psm/worktrees/omd/pr-123
```
...
Options:
  ...
  [C] Create parallel (omd:pr-123-2)
  [Q] Quit
```
...
🔍 Fetching PR #123 from oh-my-droid...
   Title: "Add webhook support for external integrations"
   Author: @contributor
   Changed: 12 files (+450, -23)

📁 Creating worktree at ~/.psm/worktrees/omd/pr-123...
   Branch: feature/webhook-support
   Base: main

🖥️  Creating tmux session: psm:omd:pr-123...

🤖 Launching Droid with PR context...

✅ Session ready!

   Session ID: omd:pr-123
   Worktree:   ~/.psm/worktrees/omd/pr-123
   Tmux:       psm:omd:pr-123

   Commands:
     /psm attach omd:pr-123  - Reattach later
     /psm kill omd:pr-123    - End session
