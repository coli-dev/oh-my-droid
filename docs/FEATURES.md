...
> Internal API documentation for oh-my-droid developers and contributors.
...
// Paths
getSpecPath(directory: string): string  // .omd/autopilot/spec.md
getPlanPath(directory: string): string  // .omd/plans/autopilot-impl.md
```
...
All state is persisted to `.omd/state/autopilot-state.json` and includes:
