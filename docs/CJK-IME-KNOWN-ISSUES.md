...
This document describes known issues with CJK (Chinese, Japanese, Korean) IME input in Droid CLI and provides workarounds for affected users.
...
Droid CLI uses React Ink for terminal UI rendering. Due to limitations in how terminal raw mode handles IME (Input Method Editor) composition events, CJK users experience various input issues ranging from invisible characters to mispositioned composition text.
...
Compose your text in an external editor that handles IME correctly, then paste into Droid.

1. Open any text editor (VS Code, Notes, TextEdit, Notepad)
2. Type your CJK text there
3. Copy (`Cmd+C` / `Ctrl+C`)
4. Paste into Droid (`Cmd+V` / `Ctrl+V`)
...
Create a script that reads from clipboard and sends to Droid:
...
xclip -selection clipboard -o | droid --stdin
```

### Workaround 4: Use IDE Integration

Use Droid through IDE integrations (VS Code extension) which may handle IME better than raw terminal.

## Related Issues

### oh-my-droid
- [#344](https://github.com/coli-dev/oh-my-droid/issues/344) - Korean IME input invisible in input field

### anthropics/droid-code
- [#22732](https://github.com/anthropics/droid-code/issues/22732) - Korean IME: Characters completely invisible during composition
- [#18291](https://github.com/anthropics/droid-code/issues/18291) - Korean IME composition: jamo not displayed until syllable completion
- [#16322](https://github.com/anthropics/droid-code/issues/16322) - [CRITICAL] Korean IME: Composing characters display at wrong position
- [#15705](https://github.com/anthropics/droid-code/issues/15705) - Korean input characters disappear on iOS mobile SSH
- [#1547](https://github.com/anthropics/droid-code/issues/1547) - IME input causes performance issues
- [#3045](https://github.com/anthropics/droid-code/issues/3045) - Investigation: Fixing IME Issues by Patching React Ink
