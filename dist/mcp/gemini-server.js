 ...
 *
 * Exposes `ask_gemini` tool via the Droid Agent SDK's createSdkMcpServer helper.
...
const askGeminiTool = tool("ask_gemini", `Send a prompt to Google Gemini CLI for design/implementation tasks. Gemini excels at frontend design review and implementation with its 1M token context window. Recommended roles: ${GEMINI_RECOMMENDED_ROLES.join(", ")}. Any valid OMD agent role is accepted. Fallback chain: ${GEMINI_MODEL_FALLBACKS.join(" → ")}. Requires Gemini CLI (npm install -g @google/gemini-cli).`, {
    agent_role: {
        type: "string",
