 ...
 *
 * Exposes `ask_codex` tool via the Droid Agent SDK's createSdkMcpServer helper.
...
const askCodexTool = tool("ask_codex", `Send a prompt to OpenAI Codex CLI for analytical/planning tasks. Codex excels at architecture review, planning validation, critical analysis, and code/security review validation. Requires agent_role to specify the perspective. Recommended roles: ${CODEX_RECOMMENDED_ROLES.join(', ')}. Any valid OMD agent role is accepted. Requires Codex CLI (npm install -g @openai/codex).`, {
    agent_role: { type: "string", description: `Required. Agent perspective for Codex. Recommended: ${CODEX_RECOMMENDED_ROLES.join(', ')}. Any valid OMD agent role is accepted.` },
