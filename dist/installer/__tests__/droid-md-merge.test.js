...
import { mergeDroidMd } from '../index.js';
...
describe('mergeDroidMd', () => {
    const omdContent = '# OMD Configuration\n\nThis is the OMD content.';
    describe('Fresh install (no existing content)', () => {
        it('wraps omdContent in markers', () => {
            const result = mergeDroidMd(null, omdContent);
            expect(result).toContain(START_MARKER);
            expect(result).toContain(END_MARKER);
            expect(result).toContain(omdContent);
            expect(result.indexOf(START_MARKER)).toBeLessThan(result.indexOf(omdContent));
            expect(result.indexOf(omdContent)).toBeLessThan(result.indexOf(END_MARKER));
        });
    ...
    describe('Update existing content with markers', () => {
        it('replaces only content between markers', () => {
            ...
            const result = mergeDroidMd(existingContent, omdContent);
            expect(result).toContain('Some header content');
            expect(result).toContain(omdContent);
            expect(result).toContain('User\'s custom content');
            expect(result).not.toContain('Old OMD Content');
    ...
    describe('No markers in existing content', () => {
        it('wraps omdContent in markers and preserves existing content after user customizations header', () => {
            ...
            // Check order: OMD section first, then user customizations header, then existing content
            const omdIndex = result.indexOf(START_MARKER);
            const customizationsIndex = result.indexOf(USER_CUSTOMIZATIONS);
            const existingIndex = result.indexOf('# My Custom Config');
            expect(omdIndex).toBeLessThan(customizationsIndex);
    ...
    describe('Edge cases', () => {
        it('handles empty omdContent', () => {
            const existingContent = `${START_MARKER}\nOld content\n${END_MARKER}`;
            const result = mergeDroidMd(existingContent, '');
            expect(result).toContain(START_MARKER);
            expect(result).toContain(END_MARKER);
    ...
    describe('Real-world scenarios', () => {
        it('handles typical fresh install scenario', () => {
            const result = mergeDroidMd(null, omdContent);
            expect(result).toMatch(/^<!-- OMD:START -->\n.*\n<!-- OMD:END -->\n$/s);
        });
...
User added custom stuff here`;
            const result = mergeDroidMd(oldContent, omdContent);
            // New OMD content should be at the top with markers
    ...
    describe('idempotency guard', () => {
        it('strips markers from omdContent that already has markers', () => {
            // Simulate docs/AGENTS.md shipping with markers already
            const omdWithMarkers = `<!-- OMD:START -->
# oh-my-droid
Agent instructions here
<!-- OMD:END -->`;
            const result = mergeDroidMd(null, omdWithMarkers);
            // Should NOT have nested markers
            const startCount = (result.match(/<!-- OMD:START -->/g) || []).length;
            const endCount = (result.match(/<!-- OMD:END -->/g) || []).length;
            expect(startCount).toBe(1);
            expect(endCount).toBe(1);
            expect(result).toContain('Agent instructions here');
        });
        it('handles omdContent with markers when merging into existing content', () => {
            const existingContent = `<!-- OMD:START -->
Old OMD content
<!-- OMD:END -->
...
New OMD content v2
<!-- OMD:END -->`;
            const result = mergeDroidMd(existingContent, omdWithMarkers);
            // Should have exactly one pair of markers
            const startCount = (result.match(/<!-- OMD:START -->/g) || []).length;
            const endCount = (result.match(/<!-- OMD:END -->/g) || []).length;
            expect(startCount).toBe(1);
            expect(endCount).toBe(1);
            expect(result).toContain('New OMD content v2');
            expect(result).not.toContain('Old OMD content');
            expect(result).toContain('My custom stuff');
        });
    });
});
//# sourceMappingURL=droid-md-merge.test.js.map