 ...
 * Based on droid-hud implementation by jarrodwatts.
 *
 * Authentication:
...
function readKeychainCredentials() {
    if (process.platform !== 'darwin')
        return null;
    try {
        const result = execSync('/usr/bin/security find-generic-password -s "Droid-credentials" -w 2>/dev/null', { encoding: 'utf-8', timeout: 2000 }).trim();
        if (!result)
            return null;
 ...
 * Updates only the droidAiOauth fields, preserving other data.
 */
