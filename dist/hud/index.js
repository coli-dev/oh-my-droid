 ...
 * Receives stdin JSON from Droid and outputs formatted statusline.
 */
...
import { readRalphStateForHud, readUltraworkStateForHud, readPrdStateForHud, readAutopilotStateForHud, } from "./omd-state.js";
...
async function main() {
    try {
        ...
        if (isInstallError) {
            ...
            console.log("[OMD] HUD error - check stderr");
            // Log actual runtime errors to stderr for debugging
            console.error("[OMD HUD Error]", error instanceof Error ? error.message : error);
        }
    }
}
// Run main
