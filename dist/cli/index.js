...
program
    .name("omd")
    .description("Multi-agent orchestration system for Droid Agent SDK with analytics")
    .version(version)
...
Examples:
  ...
  $ omd export cost csv data.csv --period weekly   Export weekly costs`)
    ...
    if (!["json", "csv"].includes(format)) {
        ...
        console.error(chalk.gray("Example: omd export sessions csv sessions.csv"));
        process.exit(1);
    }
...
Examples:
  ...
  $ omd tui --daily              Start with daily view`)
    ...
    try {
        ...
        console.error(chalk.gray('Try running "omd tui" again, or check if tokscale is properly installed.'));
        process.exit(1);
    }
});
/**
...
Examples:
  ...
  $ omd update --force           Force reinstall`)
    ...
    try {
        ...
        console.error(chalk.gray('Try again with "omd update --force", or reinstall with "omd install --force".'));
        process.exit(1);
    }
});
/**
...
Examples:
  ...
  $ omd install --quiet          Silent install for scripts`)
    ...
    if (!options.quiet) {
        console.log(chalk.blue("╔═══════════════════════════════════════════════════════════╗"));
        console.log(chalk.blue("║         Oh-My-DroidCode Installer                        ║"));
        console.log(chalk.blue("║   Multi-Agent Orchestration for Droid               ║"));
        console.log(chalk.blue("╚═══════════════════════════════════════════════════════════╝"));
        console.log("");
    }
    ...
    if (result.success) {
        ...
        console.error(chalk.gray('For more diagnostics, run "omd doctor conflicts".'));
        process.exit(1);
    }
});
/**
...
const teleportCmd = program
    ...
    .description("Create git worktree for isolated development (e.g., omd teleport #123)")
    .option("--worktree", "Create worktree (default behavior, flag kept for compatibility)")
    .option("-p, --path <path>", "Custom worktree path (default: ~/Workspace/omd-worktrees/)")
...
Examples:
  ...
  $ omd teleport remove ./path   Remove a worktree`)
    ...
    if (!ref) {
        ...
        console.log("Usage:");
        console.log("  omd teleport <ref>           Create worktree for issue/PR/feature");
        ...
        console.log("  omd teleport #42             Create worktree for issue #42");
        console.log('  omd teleport add-auth        Create worktree for feature "add-auth"');
        console.log("");
        return;
    }
