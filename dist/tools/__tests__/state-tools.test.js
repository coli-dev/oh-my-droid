...
describe('state-tools', () => {
    beforeEach(() => {
        mkdirSync(join(TEST_DIR, '.omd', 'state'), { recursive: true });
    });
    afterEach(() => {
    ...
    describe('state_read', () => {
        it('should return state when file exists at session-scoped path', async () => {
            ...
            const sessionDir = join(TEST_DIR, '.omd', 'state', 'sessions', sessionId);
    ...
    describe('state_write', () => {
        it('should write state to session-scoped path by default', async () => {
            ...
            const sessionId = getProcessSessionId();
            const sessionPath = join(TEST_DIR, '.omd', 'state', 'sessions', sessionId, 'ralph-state.json');
            expect(existsSync(sessionPath)).toBe(true);
            // Legacy path should NOT exist
            expect(existsSync(join(TEST_DIR, '.omd', 'state', 'ralph-state.json'))).toBe(false);
        });
    ...
    describe('session_id parameter', () => {
        it('should write state with explicit session_id to session-scoped path', async () => {
            ...
            expect(result.content[0].text).toContain('Successfully wrote');
            const sessionPath = join(TEST_DIR, '.omd', 'state', 'sessions', sessionId, 'ultrawork-state.json');
            expect(existsSync(sessionPath)).toBe(true);
        });
        ...
        it('should clear session-specific state without affecting legacy', async () => {
            ...
            writeFileSync(join(TEST_DIR, '.omd', 'state', 'ralph-state.json'), JSON.stringify({ active: true, source: 'legacy' }));
            const sessionDir = join(TEST_DIR, '.omd', 'state', 'sessions', sessionId);
            mkdirSync(sessionDir, { recursive: true });
            ...
            expect(existsSync(join(TEST_DIR, '.omd', 'state', 'ralph-state.json'))).toBe(true);
        });
    });
    describe('auto session ID injection (Issue #456)', () => {
        ...
        it('should write state to session-scoped path even without explicit session_id', async () => {
            ...
            const legacyPath = join(TEST_DIR, '.omd', 'state', 'ultrawork-state.json');
