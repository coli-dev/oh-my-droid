...
describe('worktree-paths', () => {
    ...
    describe('resolveStatePath', () => {
        it('should resolve state file paths with -state suffix', () => {
            ...
            expect(result).toBe(join(TEST_DIR, '.omd', 'state', 'ralph-state.json'));
        });
        it('should handle input already having -state suffix', () => {
            const result = resolveStatePath('ultrawork-state', TEST_DIR);
            expect(result).toBe(join(TEST_DIR, '.omd', 'state', 'ultrawork-state.json'));
        });
    ...
    describe('helper functions', () => {
        it('getWorktreeNotepadPath returns correct path', () => {
            ...
            expect(result).toBe(join(TEST_DIR, '.omd', 'notepad.md'));
        });
    ...
    describe('ensureAllOmcDirs', () => {
        it('should create all standard .omd subdirectories', () => {
            ...
            expect(existsSync(join(TEST_DIR, '.omd', 'research'))).toBe(true);
            expect(existsSync(join(TEST_DIR, '.omd', 'logs'))).toBe(true);
            expect(existsSync(join(TEST_DIR, '.omd', 'notepads'))).toBe(true);
            expect(existsSync(join(TEST_DIR, '.omd', 'drafts'))).toBe(true);
        });
    });
