...
describe("Ralph PRD Module", () => {
    ...
    describe("findPrdPath", () => {
        ...
        it("should find prd.json in .omd directory", () => {
            const omdDir = join(testDir, ".omd");
            mkdirSync(omdDir, { recursive: true });
            const prdPath = join(omdDir, PRD_FILENAME);
            writeFileSync(prdPath, "{}");
            expect(findPrdPath(testDir)).toBe(prdPath);
        });
        it("should prefer root over .omd", () => {
    ...
    describe("readPrd / writePrd", () => {
        ...
        it("should create .omd directory when writing", () => {
            writePrd(testDir, samplePrd);
            expect(existsSync(join(testDir, ".omd"))).toBe(true);
        });
