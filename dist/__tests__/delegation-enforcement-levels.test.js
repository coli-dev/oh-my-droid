...
import { processOrchestratorPreTool, isAllowedPath, isSourceFile, isWriteEditTool, clearEnforcementCache, } from '../hooks/omd-orchestrator/index.js';
// Mock fs module
vi.mock('fs', async () => {
...
describe('delegation-enforcement-levels', () => {
    ...
    describe('isAllowedPath', () => {
        ...
        it('handles Windows-style .omd paths', () => {
            expect(isAllowedPath('.omd\\plans\\test.md')).toBe(true);
        });
        it('rejects Windows traversal .omd\\..\\src\\file.ts', () => {
            expect(isAllowedPath('.omd\\..\\src\\file.ts')).toBe(false);
        });
        // Nested .omd in non-root position (should be rejected for relative paths)
        it('rejects foo/.omd/bar.ts as relative path', () => {
            expect(isAllowedPath('foo/.omd/bar.ts')).toBe(false);
        });
        // Windows mixed-separator edge cases
        it('rejects mixed separator traversal .omd\\..\\..\\secret', () => {
            expect(isAllowedPath('.omd\\..\\..\\secret')).toBe(false);
        });
        it('rejects double-dot with mixed separators .omd/..\\src', () => {
            expect(isAllowedPath('.omd/..\\src')).toBe(false);
        });
        it('rejects UNC paths as not relative to project', () => {
            expect(isAllowedPath('\\\\server\\share\\.omd\\file')).toBe(false);
        });
        it('rejects absolute Windows drive paths without worktree root', () => {
            expect(isAllowedPath('C:\\repo\\.omd\\file')).toBe(false);
        });
    });
