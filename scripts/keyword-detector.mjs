...
function activateState(directory, prompt, stateName, sessionId) {
  ...
  if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
    const sessionDir = join(directory, '.omd', 'state', 'sessions', sessionId);
  ...
  // Fallback: write to legacy local .omd/state directory (no valid sessionId)
...
function clearStateFiles(directory, modeNames, sessionId) {
  for (const name of modeNames) {
    ...
    if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
      const sessionPath = join(directory, '.omd', 'state', 'sessions', sessionId, `${name}-state.json`);
...
/**
 * Create a skill invocation message that tells Droid to use the Skill tool
 */
...
Steps:
1. Write a prompt file to \`.omd/prompts/${provider}-{purpose}-{timestamp}.md\` containing clear task instructions derived from the user's request
...
/**
 * Create proper hook output with additionalContext (Droid hooks API)
