...
function detectVersionDrift() {
  ...
  const droidMdVersion = getDroidMdVersion();
  ...
  if (npmVersion && npmVersion !== pluginVersion) {
    drift.push({ component: 'npm package (omd CLI)', current: npmVersion, expected: pluginVersion });
  }

  if (droidMdVersion === 'unknown') {
    drift.push({
      component: 'AGENTS.md instructions',
      current: 'unknown (needs migration)',
      expected: pluginVersion
    });
  } else if (droidMdVersion && droidMdVersion !== pluginVersion) {
    drift.push({
      component: 'AGENTS.md instructions',
      current: droidMdVersion,
      expected: pluginVersion
    });
  }
  ...
  return { pluginVersion, npmVersion, droidMdVersion, drift };
}
  ...
  try {
    ...
    try {
      ...
      if (pluginVersion) {
        ...
        if (updateInfo) {
          messages.push(`<session-restore>\n\n[OMD UPDATE AVAILABLE]\n\nA new version of oh-my-droid is available: v${updateInfo.latestVersion} (current: v${updateInfo.currentVersion})\n\nTo update, run: omd update\n(This syncs plugin, npm package, and AGENTS.md together)\n\n</session-restore>\n\n---\n`);
        }
      }
    } catch {}
...
[Sisyphus] HUD not configured (${hudCheck.reason}). Run /hud setup then restart Droid.
</system-reminder>`);
    }
    ...
    if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
      ...
      ultraworkState = readJsonFile(join(directory, '.omd', 'state', 'sessions', sessionId, 'ultrawork-state.json'));
      ...
      ultraworkState = readJsonFile(join(directory, '.omd', 'state', 'ultrawork-state.json'));
    }
...
`);
    ...
    if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
      // Session-scoped ONLY — no legacy fallback
      ralphState = readJsonFile(join(directory, '.omd', 'state', 'sessions', sessionId, 'ralph-state.json'));
      // Validate session identity
      ...
      // No session_id — legacy behavior for backward compat
      ralphState = readJsonFile(join(directory, '.omd', 'state', 'ralph-state.json'));
      if (!ralphState) {
        ralphState = readJsonFile(join(directory, '.omd', 'ralph-state.json'));
      }
    }
