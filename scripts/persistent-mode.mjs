      ...
      if (phase !== "complete") {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[AUTOPILOT - Phase: ${phase}] Autopilot not complete. Continue working. When all phases are complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`,
            }),
          );
          return;
        }
    ...
    if (ultraqa.state?.active && !isStaleState(ultraqa.state)) {
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        console.log(
          JSON.stringify({
            decision: "block",
            reason: `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`,
          }),
        );
        return;
      }
