...
function getAgentCompletionSummary(directory) {
  const trackingFile = join(directory, '.omd', 'state', 'subagent-tracking.json');
  try {
    if (existsSync(trackingFile)) {
      ...
      const parts = [];
      if (running.length > 0) {
        parts.push(`Running: ${running.length} [${running.map(a => a.agent_type.replace('oh-my-droid:', '')).join(', ')}]`);
      }
