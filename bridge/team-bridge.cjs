...
var TMUX_SESSION_PREFIX = "omd-team";
...
function logAuditEvent(workingDirectory, event) {
  ...
  const dir = (0, import_node_path.join)(workingDirectory, ".omd", "logs");
...
function writePromptFile(config, taskId, prompt) {
  const dir = (0, import_path6.join)(config.workingDirectory, ".omd", "prompts");
  ensureDirWithMode(dir);
