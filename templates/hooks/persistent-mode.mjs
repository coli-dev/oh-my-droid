...
/**
 * Count incomplete Tasks from Droid's native Task system.
 */
 ...
 * When context is exhausted, Droid needs to stop so it can compact.
 * Blocking these stops causes a deadlock: can't compact because can't stop,
 * can't continue because context is full.
 *
 * See: https://github.com/coli-dev/oh-my-droid/issues/213
 */
...
async function main() {
  try {
    ...
    // Blocking these causes a deadlock where Droid cannot compact.
    // See: https://github.com/coli-dev/oh-my-droid/issues/213
    ...
    if (
      ...
      if (iteration < maxIter) {
        ...
        let reason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /oh-my-droid:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /oh-my-droid:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
        if (errorGuidance) {
    ...
    if (
      ...
      if (phase !== "complete") {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[AUTOPILOT - Phase: ${phase}] Autopilot not complete. Continue working. When all phases are complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        let reason = `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-droid:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-droid:cancel --force.`;
        if (errorGuidance) {
          reason = errorGuidance + reason;
        }
    ...
    try {
      ...
      // If stdout write fails, the hook will timeout and Droid will proceed
