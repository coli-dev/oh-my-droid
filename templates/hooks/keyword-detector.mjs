...
2. Write a prompt file to \`.omd/prompts/${provider}-{purpose}-{timestamp}.md\` containing clear task instructions derived from the user's request
...
If ToolSearch returns no MCP tools, the MCP server is not configured. Fall back to the equivalent Droid agent instead.
