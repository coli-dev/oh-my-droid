...
### Changed
- Migrated project naming and branding from Droid/OMC to Droid/OMD, including repo/org references, commands, config paths, and plugin metadata.
- Renamed AI instruction reference from `DROID.md` to `AGENTS.md` and updated associated setup/installer flows.
...
### Changed
- **Team skill**: Removed hardcoded `model: "sonnet"` default for team members. Teammates now inherit the user's session model instead of being forced to Sonnet. Since each teammate is a full Droid session capable of spawning its own subagents, the session model serves as the orchestration layer.
- **Team config**: Removed `defaultModel` from `.omd-config.json` team configuration options.

---

# oh-my-droid v4.1.1: Session Isolation & Flexible MCP Routing
...
- **Agent Teams Setup**: The `omd-setup` wizard now includes Step 5.5 to configure Droid's experimental Agent Teams (`DROID_CODE_EXPERIMENTAL_AGENT_TEAMS`), including `teammateMode` selection and team defaults. (#484)
...
- **Session State Isolation**: Eliminated cross-session state contamination when multiple Droid sessions run in the same directory. When `session_id` is known, legacy shared state is invisible — no fallback to shared paths. Adds `isSessionMatch()` helper for consistent session matching across all 8 modes. (#486)
...
# oh-my-droid v4.1.0: The Consolidation & Coordination Update
...
In line with Anthropic's latest prompting best practices, the core system prompt (`docs/AGENTS.md`) has been completely rewritten for significantly improved performance, reliability, and tool-use accuracy.
...
- **Merged Skills**:
  ...
  - `learn-about-omd` is now part of the `help` command.
