...
```
/oh-my-droid:pipeline review
```

Or demonstrate custom pipeline:
```
/oh-my-droid:pipeline explore:haiku -> architect:opus -> critic:opus -> executor:sonnet
```
...
```
/oh-my-droid:pipeline explore:haiku -> architect:opus -> executor-high:opus -> qa-tester:sonnet
```
...
```bash
cat .omd/state/pipeline-state.json
```
