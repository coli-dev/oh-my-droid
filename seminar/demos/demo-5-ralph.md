...
```bash
cat .omd/state/ralph-state.json
```
...
Together, they transform Droid from a helpful assistant into a development team. Questions?"
...
A: Yes! Check `.omd/state/ralph-state.json` for iteration log, or use verbose mode: "ralph --verbose: refactor X"

**Q: What happens if I cancel Ralph mid-iteration?**
A: State is saved. Resume with "resume ralph" or "/oh-my-droid:resume-session". It picks up where it left off.
