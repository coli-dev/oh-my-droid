...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Run complete benchmark comparison between vanilla and OMD modes."
            echo ""
            echo "Options:"
            echo "  --limit N         Limit to N instances (default: all)"
            echo "  --skip N          Skip first N instances (default: 0)"
            echo "  --model MODEL     Droid model to use (default: claude-sonnet-4.5-20250929)"
...
if [ "$SKIP_VANILLA" = false ]; then
    log_step "Step 1/4: Running vanilla Droid benchmark..."
    echo ""
